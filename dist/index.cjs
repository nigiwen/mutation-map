...
const d3__namespace = /*#__PURE__*/_interopNamespaceCompat(d3);
...
class MutationMap {
  ...
  #createSvgContainer() {
    return d3__namespace.create("svg").attr("font-family", `"Source Sans Pro", "Helvetica Neue", Helvetica`).style("vertical-align", "middle");
  }
  #createGContainer() {
  ...
  render() {
    ...
    this.#renderContent();
    this.#renderCursor();
    d3__namespace.select(this.#rootContainer).append(() => this.#svgContainer.node());
  }
}
const createMutationMap = (el) => new MutationMap(el);

exports.createMutationMap = createMutationMap;
